...
export class MediaService {
  ...
  getMediaInternal(request: GetMediaRequest): GetMediaResponse {
    try {
      ...
      throw new NotFoundException(
        MEDIA_CONSTANTS.ERROR_MESSAGES.GET_MEDIA_FAILED,
        error,
      );
    }
  }

  async getSignedUrlsInternal(
    ...
    try {
      const urls = await Promise.all(
        request.files.map(async (file) => {
          ...
          const uploadUrl =
            await this.s3Service.generatePresignedPutUrl(fileKey);
      ...
      throw new NotFoundException(
        MEDIA_CONSTANTS.ERROR_MESSAGES.GENERATE_URLS_FAILED,
        error,
      );
    }
  }

  async deleteMediaInternal(
      ...
      return {
        ...
        message:
          failedFiles.length > 0
            ...
            : MEDIA_CONSTANTS.RESPONSE_MESSAGES.DELETE_SUCCESS(
                deletedFiles.length,
              ),
      };
    } catch (error) {
